...
There are **38 free connected KPIs** through FRED and, for five Treasury rate indicators, the U.S. Treasury feed, plus **12 CSV import slots** and two outcome/benchmark series. Availability varies by series; a working connection does not imply unrestricted or sufficiently long history. Prices reflect the research checked on 12 September 2026; the UI labels annualized monthly rates and observed regional offers separately from annual billing. No paid account is connected or purchased.
...
- Importing replaces that indicator's cache. Routine watchlist and daily refreshes retain imported data. An individual indicator's **Refresh** button, or the event study's explicit refresh, replaces its data with connected provider observations (Treasury recent rates and FRED history for the five Treasury KPIs).
...
FRED requests send the requested series IDs and date range to St. Louis Fed services. Treasury rate requests go to the official Treasury feed. First-release API requests also send your FRED key to the official API over HTTPS. The app has no analytics, LLM integration, advertising or cloud sync. Its optional refresh task runs only while the application or web server is running. It opens external source links only after a click. Its renderer cannot directly access the filesystem or the network.
...
Automatic refresh is off by default. When enabled, each selected Treasury indicator is checked hourly and other connected KPIs after 23 hours. The first check runs when the app opens. Desktop checks require the app to be running; web checks run on the server while it is running, even without an open browser tab. Sleeping Replit Autoscale deployments resume checks when the app is opened. Imported series are preserved.

The 10-year and 2-year nominal yields, 10-year TIPS real yield, and 10Y–2Y/10Y–3M spreads use the [official Treasury daily XML feed](https://home.treasury.gov/treasury-daily-interest-rate-xml-feed) for recent dates. FRED supplies long history (and optional dated first releases). If a provider fails, an existing cache is retained, the displayed source is identified, and old data is marked **Check freshness** after a missing completed U.S. business day. The detail panel shows the observation date, most recent source, last retrieval, and any Treasury fallback warning. In first-release mode, recent Treasury points are dated no earlier than when MacroSignals first retrieved them; their first-seen values are retained until FRED first-release history catches up.
