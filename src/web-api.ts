...
setInterval(async()=>{if(document.visibilityState!=='visible'||refreshing||!latest?.state.autoRefresh)return;const r=await webApi.bootstrap();if(!r.ok)return;const data=r.result!;if(data.state.lastRefresh&&Date.now()-Date.parse(data.state.lastRefresh)<23*3600000)return;const ids=[...data.state.selected.filter(id=>data.catalogue.find(k=>k.id===id)?.adapter==='fred'),'market','recession'].filter(id=>!['user-first-release','import-revised'].includes(data.cache[id]?.historyQuality));const refreshed=await webApi.refresh(ids);emit(refreshed.ok?{phase:'updated',...refreshed.result}:{phase:'error',error:refreshed.error});},60000);
